...
variant {
	vertex {
		...
		source %{
			...
			//%normal% = _mtx_mul(vNormalViewMatrix , axeZ); 
			//vertice_pos = rot * (vPosition * scale);
			...
			//%out.position% = _mtx_mul(vViewMatrix * vProjectionMatrix, vec4(vertice_world, 1.0));
		%}
	}
	...
	pixel {
		...
		source %{
			...
			%diffuse% = vec3(0,0,0);
			%specular% = vec3(0.,0.,0.);
			float a = c_texture.a*teint.a*altitude_alpha; //*a_dist;
			%opacity% = a; 
